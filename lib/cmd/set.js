...
const outputVariables = (pattern) => {
	...
	const mapper = (key) => ({
		title: key,
		subtitle: "⇒ " + arvish.config.get(key),
		valid: false,
		autocomplete: `!set ${key} `,
	});

	const out = arvish.matches(pattern, Object.keys(config.defaults)).map(mapper);
		...
		return [
			{
				...
				subtitle: `Old value ⇒ ${arvish.config.get(variableName)}`,
				valid: true,
				arg: JSON.stringify({
					alfredworkflow: {
						variables: {
